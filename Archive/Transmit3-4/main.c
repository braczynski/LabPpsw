...
int main(){
	...
	unsigned char ucTransmitString[TRANSMITER_SIZE] = "licznik ";
	...
	while(1) {
		...
		if(Transmiter_GetStatus() == FREE) {
			...
			AppendUIntToString(uiTransmitCounter, (char *)ucTransmitString);
			
			Transmiter_SendString(ucTransmitString);
			uiTransmitCounter++;
		} 
		
		ucTransmitString[8] = NULL;
		iMainLoopCtr++;
	}
}
